	...
	$('.export_csv').click(function () {
		...
		if ('table' === export_format) {
			...
			if (!window.main_chart) {
				if (jQuery('ul.card_body_list.top_sellers li').length > 0) {
					$('ul.card_body_list.top_sellers li').each(function () {
						$(this).find('.top-seller-elm').each(function () {
							var value = $(this).text();
							value = value.replace('[?]', '').replace('#', '');
							csv_data += '"' + value + '"' + ',';
						});
					...
					var value = jQuery('ul.card_body_list.top_sellers').text();
					value = value.replace('[?]', '').replace('#', '');
					csv_data += '"' + value + '"' + ',';
					csv_data = csv_data.substring(0, csv_data.length - 1);
					csv_data += '\n';
